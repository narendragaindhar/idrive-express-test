module API
  module V3
    class OrdersController < API::APIController
      # POST /api/v3/orders (JSON) accepts the following params:
      # {
      #   order: {
      ...
      def create
        ...
        if @order.save
          ...
          logger.error "API::V3 422 Error: Could not save order because: #{@order.errors.full_messages}"
          render json: @order.errors, status: :unprocessable_entity
        end
      rescue ActionController::ParameterMissing => e
        logger.error "API::V3 API 400 Error: Could not save order because: #{e}"
      ...
      def order_type_params
        p = params.require(:order).require(:order_type).permit(:key)
        if p[:key] == 'upload'
